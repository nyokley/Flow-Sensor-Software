...
void main(void)
{
	...
	while(1)
	{
		...
		if(eTempNFCState == NFC_DATA_EXCHANGE_PROTOCOL)
		{
			...
			else if(NFC_P2P_getModeStatus(&sP2PMode,&sP2PBitrate))
			{
				...
				if(sP2PRxStatus.sDataReceivedStatus != RECEIVED_NO_FRAGMENT)
				{
					...
					int i = 0;
                    reader.receivedStringLength = *(sP2PRxStatus.pui8RxDataPtr+2) - 3;
                    for(i = 0; i < reader.receivedStringLength; ++i) {
                        reader.receivedString[i] = *(sP2PRxStatus.pui8RxDataPtr+7+i);
                        reader.receivedTx = 1;
                    }
		...
		if(reader.receivedTx) {
		    char tempString[100];
		    strncpy(tempString, reader.receivedString, reader.receivedStringLength);
		    Serial_printf(tempString, NONE);
		    reader.receivedTx = 0;
		}

		switch(reader.currState) {
		case SEND_COMMAND:
		    reader.transmitString[0] = reader.command;
		    switch(reader.command) {
		    case START_POLL:
		        //bytes1-4: number of samples to take
		        reader.transmitStringLength = 5;
		        break;
		    default:
		    }
		    transmitMessage(reader.transmitString,reader.transmitStringLength);
		    reader.confirmed = 0;
		    reader.currState = WAIT_CONFIRM;
		    break;
		case SEND_CONFIRM:
		    reader.transmitString[0] = CONFIRM;
		    break;
		case WAIT_RESPONSE:
		    break;
		case WAIT_CONFIRM:
		    if(reader.confirmed) {
		        switch(reader.command) {
		        case READ_DSTATUS:
		            reader.currState = WAIT_RESPONSE;
		            break;
		        }
		    }
		    else if(reader.confirm1Incrementer > reader.confirm1Max) {
		        reader.confirm2Incrementer++;
		        reader.confirm1Incrementer = 0;
		        reader.currState = SEND_COMMAND;
		    }
		    else if(reader.confirm2Incrementer > reader.confirm2Max) {
		        //abort
		    }
		    break;
		case DELAY:
		    if(reader.delayIncrementer > reader.delayMax) {
		        reader.currState = reader.nextState;
		        reader.delayIncrementer = 0;
		    }
		    else{
		        reader.delayIncrementer++;
		    }
		    break;
		case IDLE:
		    break;
...
void Serial_processCommand(void)
{
    ...
    reader.command = (tReaderImplantCommands) g_ui8SerialBuffer[0];
    reader.currState = SEND_COMMAND;

    switch(g_ui8SerialBuffer[0]) {
    case START_POLL:
        strcpy(reader.serialString, "Starting Poll");
        break;
    case STOP_POLL:
        strcpy(reader.serialString, "Stopping Poll");
        break;
    case READ_DATA:
        strcpy(reader.serialString, "Reading Data");
        break;
    case READ_DSTATUS:
        strcpy(reader.serialString, "Reading Data Status");
        break;
    case READ_BSTATUS:
        strcpy(reader.serialString, "Reading Battery Status");
        break;
    default:
        strcpy(reader.serialString, "Not a Valid Command");
        reader.currState = IDLE;
    }
    Serial_printf(reader.serialString, NONE);
