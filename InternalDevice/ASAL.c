...
*   Author: Nathaniel
 *
 *  Contains functions specific to target-initiator communication and
 *  overall code flow
 *
 */
...
void compressData(uint16_t array[], uint8_t** targetArray) {
    uint8_t arrayC[172];
    int offset = 0;
    int mod = 0;
    int i;
...
//sends just sampleArray1 - edit to also transmit tempData
void sendData(){
    uint8_t arrayC[172];
        int offset = 0;
        int mod = 0;
        ...
        for(i = 0; i < 172; ++i) {
            switch(mod) {
            case 0:
                arrayC[i] = (uint8_t) (sensorState.sampleArray1[i-offset] & 0x00FF);
            case 1:
                arrayC[i] = (uint8_t) (((sensorState.sampleArray1[i-offset-1] & 0x0F00) >> 8) | ((sensorState.sampleArray1[i-offset] & 0x000F) << 4));
            case 2:
                arrayC[i] = (uint8_t) (sensorState.sampleArray1[i-offset-1] & 0x00FF);
                offset++;
            }
            mod = (mod+1)%3;
        }
